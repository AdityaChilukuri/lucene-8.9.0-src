...
public final class ImpactsDISI extends DocIdSetIterator {
  ...
  private int advanceTarget(int target) throws IOException {
    ...
    maxScore = maxScoreCache.getMaxScoreForLevel(0); // Adi: Compute next block's max score.
    // We keep a cache to avoid recomputing these scores. If cache miss, go to the postings skip list and read it out.
    while (true) {
      assert upTo >= target;

      if (maxScore >= minCompetitiveScore) { // Adi: The next "block" has competitive scores so do not skip
        return target;
      }
      // Adi: If made it this far in the code, then we need to skip the current block.
      ...
      upTo = advanceShallow(target);
      maxScore = maxScoreCache.getMaxScoreForLevel(0); // Adi: Compute next "block"s maxScore.
    }
  }
