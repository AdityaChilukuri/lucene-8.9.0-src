...
public final class Lucene84PostingsReader extends PostingsReaderBase {
  ...
  final class BlockImpactsDocsEnum extends ImpactsEnum {
    ...
    @Override
    public int freq() throws IOException { // Called by TermScorer.score() method. May need to decode frequency list from index
      if (isFreqsRead == false) {
        pforUtil.decode(docIn, freqBuffer); // read freqBuffer for this block
        isFreqsRead = true;
      }
    ...
    private void refillDocs() throws IOException {
      ...
      if (left >= BLOCK_SIZE) {
        forDeltaUtil.decodeAndPrefixSum(docIn, accum, docBuffer); // Adi: Decode document posting list into docBuffer containing 128 docIDs as a long[128] array
        if (indexHasFreqs) {
          pforUtil.decode(docIn, freqBuffer); // Adi: Decode document frequencies list as another long[128] array
        }
        blockUpto += BLOCK_SIZE;
      } else {
